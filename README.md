...
Newscrux monitors 13 AI/ML RSS feeds, filters articles by relevance using AI, extracts full article content when needed, generates structured summaries in your chosen language, and delivers them as rich push notifications to your phone via [Pushover](https://pushover.net).
...
- 🧠 **Structured summaries** — What happened + Why it matters + Key detail, via any OpenAI-compatible LLM
...
- An OpenAI-compatible LLM API endpoint — any of:
  - [OpenRouter](https://openrouter.ai/keys) (free tier available)
  - Self-hosted [llama.cpp](https://github.com/ggerganov/llama.cpp), [vLLM](https://github.com/vllm-project/vllm), [Ollama](https://ollama.com), etc.
...
To use a local llama.cpp / vLLM / Ollama server instead of OpenRouter:

```env
LLM_BASE_URL=http://192.168.1.111:8000/v1
LLM_API_KEY=not-needed          # set any non-empty value
LLM_MODEL=my-local-model
PUSHOVER_USER_KEY=...
PUSHOVER_APP_TOKEN=...
```

Any endpoint that implements OpenAI's `/v1/chat/completions` contract will work.
...
6. **Summarize** — Sends article content to the configured LLM with a structured JSON prompt in the selected language. Output: `translated_title`, `what_happened`, `why_it_matters`, `key_detail`, `source_type`.
